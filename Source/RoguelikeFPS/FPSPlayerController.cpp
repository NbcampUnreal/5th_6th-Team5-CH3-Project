...
AFPSPlayerController::AFPSPlayerController()
	...
	ShopWidget(nullptr)
	//,TitleWidgetClass(nullptr) // Title Ï∂îÍ∞Ä
...
void AFPSPlayerController::BeginPlay()
{
	...
	// {
		...
		// {
			...
			// // ∏∂øÏΩ∫ ƒøº≠ º≥¡§ (∞‘¿” ¡ﬂø°¥¬ ¿œπ›¿˚¿∏∑Œ º˚±Ë)
			// bShowMouseCursor = false;
			// FInputModeGameOnly InputMode;
			// SetInputMode(InputMode);

			// // 3. ƒ≥∏Ø≈Õ ¬¸¡∂ »πµÊ π◊ UStatsHUDø° ¿ß¡¨ º≥¡§
			// if (AFPSCharacter* MyCharacter = Cast<AFPSCharacter>(GetCharacter()))
			// {
				// // UStatsHUD∑Œ ƒ≥Ω∫∆√«œø© ƒ≥∏Ø≈Õ∏¶ º≥¡§«’¥œ¥Ÿ.
				// // UStatsHUD¥¬ SetOwningCharacterø°º≠ ƒ≥∏Ø≈Õ¿« µ®∏Æ∞‘¿Ã∆Æ∏¶ ¡˜¡¢ πŸ¿Œµ˘«’¥œ¥Ÿ.
	...
	if (PartMenuWidgetClass)
	{
		...
		if (PartMenuWidget)
		{
			...
			{
				UInventory* Inven = MyPawn->FindComponentByClass<UInventory>();
				UPartSystem* PartSys = MyPawn->FindComponentByClass<UPartSystem>(); // ‚úÖ ÏàòÏ†ïÎê®!
